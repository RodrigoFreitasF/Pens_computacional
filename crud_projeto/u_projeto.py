...
class AlterarProjeto:
    def __init__(self, janela_mestre, idt):
        ...
        lb_datainicial.grid(row=linha, column=0, padx=PADX, pady=PADY)

        self.entry_data_inicio = DateEntry(self.popup, font='Helvetica 12', width=12)
        self.entry_data_inicio.grid(row=linha, column=1, padx=PADX, pady=PADY)
        linha += 1

        lb_datafinal = tk.Label(self.popup, text="Data final do projeto", font='Helvetica 12 bold', fg='blue')
        lb_datafinal.grid(row=linha, column=0, padx=PADX, pady=PADY)

        self.entry_data_fim = DateEntry(self.popup, font='Helvetica 12', width=12)
        self.entry_data_fim.grid(row=linha, column=1, padx=PADX, pady=PADY)
        linha += 1
    ...
    def alterar(self, janela_mestre):
        ...
        if retorno[0]:
            ...
            dta_ini_projeto = self.entry_data_inicio.get_date()  # Pega a data inicial
            dta_fim_projeto = self.entry_data_fim.get_date()  # Pega a data final
            ...
            messagebox.showerror("Erro: Campos obrigatórios", "O(s) seguinte(s) campo(s) é/são obrigatório(s):\n" + retorno[1])
